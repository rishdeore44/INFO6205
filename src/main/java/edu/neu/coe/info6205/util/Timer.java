...
public class Timer {
	...
	public <T, U> double repeat(int n, Supplier<T> supplier, Function<T, U> function, UnaryOperator<T> preFunction, Consumer<U> postFunction) {
		...
		for (int i = 0; i < n; i++) {
			pause();
			T t = supplier.get();
			t = checkPreFunction(preFunction, t);
			resume();
			U u = function.apply(t);
			pause();
			checkPostFunction(postFunction, u);
			resume();
			lap();
		}
		pause();
	...
	private <U> void checkPostFunction(Consumer<U> postFunction, U u) {
		if (postFunction != null) {
			postFunction.accept(u);
		}
	}

	private <T> T checkPreFunction(UnaryOperator<T> preFunction, T t) {
		if (preFunction != null) {
